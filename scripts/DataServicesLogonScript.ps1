...
Workflow DatabaseDeploy
{
    Parallel {
        InlineScript {
            ...
            # kubectl exec $podname -n $env:arcDcName -c postgres -- /bin/bash -c "cd /tmp && curl -k -O https://raw.githubusercontent.com/likamrat/arcbox-1/replace_func/scripts/AdventureWorks.sql" 2>&1 $null
